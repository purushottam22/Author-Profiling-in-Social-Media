...
def makimg_files(preds, Test_Id, f_name):
    ...
    for final_list1 in Final_list_new:
        ...
        export_csv = clean.to_csv (f_name,index = None, header = True)



def makimg_files_gender(preds, Test_Id, f_name):
    #preds = loaded_model.predict([X1_test, Test_features])
    #print(preds)
    Id_test_list = []
    Id_test_list.append(Test_Id)
...
if __name__ == '__main__':
    ...
    #Test_file = '/home/purushottam/Desktop/IIT_patna/FIRE/Author_profiling/Data_27 (copy).csv'
   ...
   # embedding_layer = Embedding(nb_words, embedding_vecor_length, weights=[embedding_matrix],  input_length=max_tweet_length)
    ...
    print('gen train 3', gen_train3)

    model_1 = Sequential()
    
    model_1.add(Embedding(nb_words, embedding_vecor_length, weights=[embedding_matrix],  input_length=max_tweet_length))
    model_1.add(LSTM(10))

    model_2 = Sequential()
    model_2.add(Dense(10, input_dim = 10, activation = 'relu'))

    #model = Concatenate([model_1, model_2])
    model = Sequential()
    ...
    model.fit([X1_train, Train_features], gen_train3, validation_split= 0.2, epochs=20, batch_size = 256, callbacks=[earlyStop])
    preds = model.predict([X1_test, Test_features])
    makimg_files_gender(preds, Test_Id, 'Predicted/Predicted_Gender_18.csv')


    model_json = model.to_json()
    with open("Models/model_Gender_18.json", "w") as json_file:
        json_file.write(model_json)
    # serialize weights to HDF5
    model.save_weights("Models/model_Gender_18.h5")
    ...
    earlyStop=EarlyStopping(monitor="val_loss",verbose=2,mode='min',patience=3)
    model.fit([X1_train, Train_features], age_train3, validation_split= 0.2, epochs=20, batch_size = 256, callbacks=[earlyStop])
    preds = model.predict([X1_test, Test_features])
    makimg_files(preds, Test_Id, 'Predicted/Predicted_Age_18.csv')


    model_json = model.to_json()
    with open("Models/model_Age_18.json", "w") as json_file:
        json_file.write(model_json)
    # serialize weights to HDF5
    model.save_weights("Models/model_Age_18.h5")
    print("Saved model to disk")
    ...
    earlyStop=EarlyStopping(monitor="val_loss",verbose=2,mode='min',patience=3)
    model.fit([X1_train, Train_features], lang_train3, validation_split= 0.2, epochs=20, batch_size = 256, callbacks=[earlyStop])
    #earlyStop=EarlyStopping(monitor="val_loss",verbose=2,mode='min',patience=3)
    ...
    preds = model.predict([X1_test, Test_features])
    makimg_files(preds, Test_Id, 'Predicted/Predicted_Lang_18.csv')
    # serialize model to JSON
    model_json = model.to_json()
    with open("Models/model_Lang_18.json", "w") as json_file:
        json_file.write(model_json)
    # serialize weights to HDF5
    model.save_weights("Models/model_Lang_18.h5")
    print("Saved model to disk")
    '''
    # later...
